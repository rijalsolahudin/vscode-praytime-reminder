...
export function registerCommands(context: vscode.ExtensionContext) {
  const openSettings = vscode.commands.registerCommand(
    'praytime-reminder.openSettings',
    async () => {
      ...
      const adzanAudioPath = vscode.Uri.file(
        path.join(__dirname, 'assets/adzan-mekkah.mp3')
      );
      const adzanAudioUrl = panel.webview
        .asWebviewUri(adzanAudioPath)
        .toString();
  ...
  const testAdzan = vscode.commands.registerCommand(
    'praytime-reminder.testAdzan',
    async () => {
